...
<p>
  ...
  превращает HTML, CSS и JavaScript в пиксели на экране. Анимация — это тот же конвейер, пройденный заново на каждом кадре.
</p>

<p class="info">
  <strong>Главная мысль:</strong> чем меньше этапов конвейера затронуто на каждом кадре, тем плавнее анимация. Поэтому
  <strong>анимируй только <code>transform</code> и <code>opacity</code></strong> — единственные свойства, которые пропускают
  дорогие этапы Layout и Paint и обходятся почти бесплатным Composite.
</p>
...
<p class="info info--blue">
  <strong>Правило каскада:</strong> этапы срабатывают «сверху вниз»: запустил Layout — за ним обязательно идут Paint и
  Composite; запустил Paint — за ним Composite. Наоборот нельзя. Вся оптимизация анимаций — про то, чтобы остаться на самом
  нижнем этапе.
</p>
...
<h3>Бюджет кадра: 60 FPS и 16.7 мс</h3>

<p>
  Экран обновляется 60 раз в секунду, значит на кадр у браузера есть <strong>1000 / 60 ≈ 16.7 мс</strong> — и в них должны
  уложиться JavaScript, стили, Layout, Paint и Composite. Не уложился — кадр пропущен (<strong>dropped frame</strong>), и
  пользователь видит рывок (jank). Анимация <code>transform</code> дешёвая именно поэтому: она почти не тратит этот бюджет.
</p>
...
<code class="code">
  /* Плохо: каждый кадр Layout + Paint */
  .box { transition: left 300ms; }
  .box:hover { left: 300px; }

  /* Хорошо: каждый кадр только Composite */
...
<p>
  Для анимации на этапе Composite браузер выносит элемент в отдельный слой (текстуру на GPU). Подсказка
  <code>will-change: transform</code> предупреждает его об этом заранее.
</p>

<p class="info info--orange">
  Но <code>will-change</code> — это не «сделать быстро», а обещание браузеру: каждый слой ест память GPU. Навесишь на всё
  подряд «на всякий случай» — получишь падение производительности вместо роста. Ставь точечно, только на то, что вот-вот
  будет анимироваться.
</p>
...
<h4>Бюджет кадра: обрыв вместо деградации</h4>

<p>
  Кадры синхронизированы с обновлением экрана (vsync), поэтому между 60 FPS и 30 FPS нет плавного перехода — есть обрыв:
  либо кадр уложился в 16.7 мс и попал в текущее обновление экрана, либо ждёт следующего. Чуть-чуть не успевающая анимация
  визуально проседает сразу вдвое.
</p>
