<p>
  ...
  перетворює HTML, CSS і JavaScript на пікселі на екрані.
</p>

<p>
  Головна думка: <strong>анімація — це той самий конвеєр, пройдений заново на кожному кадрі</strong>. Отже, чим менше
  етапів конвеєра ми чіпаємо на кожному кадрі, тим плавніша анімація. Звідси єдиний практичний висновок, який
  закриває більшу частину питання:
</p>

<p class="info info--blue">
  <strong>Анімуй лише <code>transform</code> і <code>opacity</code>.</strong> Тільки ці дві властивості дозволяють
  пропустити найдорожчі етапи конвеєра — Layout і Paint — і працювати майже безкоштовно.
</p>
...
<p class="info info--orange">
  <strong>Правило каскаду:</strong> етапи спрацьовують «згори вниз». Запустив Layout — за ним <strong>обов'язково</strong>
  йдуть Paint і Composite. Запустив Paint — за ним іде Composite. А ось навпаки не можна: можна зробити лише Composite,
  не чіпаючи Layout і Paint. Уся оптимізація анімацій — про те, щоб залишитися на найнижчому етапі.
</p>

<h4>Що який етап запускає</h4>
...
<h3>Бюджет кадра: чому все впирається в 16.7 мс</h3>

<p>
  Екран оновлюється 60 разів на секунду, отже на один кадр у браузера є <strong>1000 / 60 ≈ 16.7 мс</strong>. І це бюджет
  не лише на твій код — у ці ж 16.7 мс браузер має вкласти стилі, Layout, Paint, Composite та збирання сміття.
</p>

<p>
  Не вклався в кадр — кадр пропущено (<strong>dropped frame</strong>), і користувач бачить ривок (jank). Анімація
  <code>transform</code> дешева саме тому: вона майже не витрачає цей бюджет.
</p>

<p class="info info--blue">
  Між 60 FPS і 30 FPS немає плавного переходу — є обрив. Або кадр вкладається в бюджет, або ні.
</p>
...
<h4>Погано — через геометрію (кожен кадр Layout + Paint)</h4>
...
<h4>Добре — через transform (кожен кадр лише Composite)</h4>
...
<h4>will-change — підказка браузеру</h4>

<p>
  Щоб анімувати елемент лише на етапі Composite, браузер виносить його в окремий шар (текстуру на GPU). Властивість
  <code>will-change</code> попереджає про це заздалегідь:
</p>
...
<p class="info info--orange">
  Але <code>will-change</code> — це не «зробити швидко», а обіцянка браузеру. Кожен шар їсть пам'ять GPU. Навісиш його
  на все підряд «про всяк випадок» — отримаєш падіння продуктивності замість зростання. Став точково, лише на те,
  що реально ось-ось буде анімуватися.
</p>
